    ...
    switch ($_GET['apicall']) {
        ...
        case 'getTransTour':
            if (isTheseParametersAvailable(array('id_user'))) {
                $id_user = $_POST['id_user'];
                $stmt = $conn->prepare("SELECT tb_tour_package.`name`, tb_tour_package.`desc`, tb_trans_tour.* FROM tb_trans_tour 
                ...
                if ($stmt->execute()) {
                    $stmt->bind_result($id, $name, $desc, $tour_date, $total_price, $id_user, $id_tour_package, $qty, $reserve_date, $status);
                    while ($stmt->fetch()) {
                        $user[] = array(
                            'id' => $id,
                            'name' => $name,
            ...
            if (isset($_POST['id_transaksi']) and isset($_FILES['image']['name'])) {
                ...
                try {
                    ...
                    if (move_uploaded_file($tmp_image, $file_url)) {
                        $sql = "UPDATE tb_trans_tour SET proof = '$file_url' WHERE id = '$id_transaksi';";
